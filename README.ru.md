...
**Reset выполняется после вашего `tearDown()`.** PHPUnit зовёт `#[After]`-хуки
после завершения `tearDown()`, а лог вызовов удерживает каждое возвращённое
значение до этого reset — то есть значение, которое дубль вернул, всё ещё
удерживается, пока идёт ваш teardown. Для значения, владеющего OS-ресурсом —
стрим, соединение, лок, — ресурс всё ещё занят: forwarding-дубль, возвращавший
...
Переопределение, забывшее композицию, не молчит: тело, которое прошло,
держало дубли и не дошло до верификации трейта, падает в хуке `#[After]` с
этим рецептом в сообщении — а контекст всё равно сбрасывается.
